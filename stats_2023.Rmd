...
tidy_distinct <- tidy_all %>%
  distinct() ###Does not change anything. Need to specify the lat long. See next area.
```
###Distinct With Specifying Lat/Long
```{r}
tidy_distinct2 <- tidy_all %>%
  distinct(lat, long, .keep_all = TRUE) ###Outputs the same thing that we previously did!
...
tidy_rounded <- tidy_rounded %>%
  distinct(rounded_lat, rounded_long, .keep_all = TRUE) ###Outputs the same as previous work!
...
tidy_rounded2 <- tidy_rounded2 %>%
  distinct(rounded_lat, rounded_long, .keep_all = TRUE) ###Outputs 420 datapoints only.
